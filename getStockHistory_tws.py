...
#   https://www.twse.com.tw/fund/BFI82U?response=html&dayDate=20210112  三大法人買賣金額
def get_big3_history(date):
    url = f'https://www.twse.com.tw/fund/BFI82U?response=html&dayDate={date}'
    print(url)
...
def create_df(date, stock_no):
    ...
    s.columns = ['date', 'shares', 'amount', 'open',
...
def genDaysInMonth(curmonth):
    days = calendar.monthrange(curmonth.year, curmonth.month)[1]
    d = []
    for i in range(days):
        fmtDate = datetime.date(curmonth.year,curmonth.month,i+1).strftime('%Y%m%d')
        d.append(fmtDate)
    return d
...
=======
    dbpath = os.path.join(dbfolder, 'tw_{}.db'.format(stockNOs[i]))
>>>>>>> 0a331483ea73fda765e41acec7251ab7f5944dd9
    ...
    for m in monthArr:
        try:
            ...
            if reqCounts < 3:
                reqCounts += 1
                time.sleep(delaytime+random.random()*RANDOMSEC)
            else:
                reqCounts = 0
                time.sleep(delaytime+random.random()*RANDOMSEC)

            # get big 3 amount 
            # days = genDaysInMonth(datetime.datetime.strptime(m, r'%Y%m%d').date())
            # for d in days:
            #     get_big3_history(d)
